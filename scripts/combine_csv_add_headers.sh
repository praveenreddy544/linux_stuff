...
if [ $? -eq 0 ];then
    ...
    a=$'"Name","Organization->Name","RAM","CPU","Application Info","Description","Additional Notes","OS family->Name","OS version->Name","Business criticality","Status","Assigned IPs Info"'
    ...
    if [ $? -ne 0 ];then
        ...
        b=$(echo $'"Name","Organization->Name","RAM","CPU","Application Info","Description","Additional Notes","OS family->Name","OS version->Name","Business criticality","Status","Assigned IPs Info"')
        sed  -i "1i $b" $source
    fi
fi
fi
